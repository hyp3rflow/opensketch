...
- [x] **Flow Lint Scope Preset Cleanup (2026-04-21)**: Flow Lint Scope 영역에 preset/quick slot usage 카운트(Preset·Quick Slot별 S/P/F 분포)와 stale key 개수를 표시한다. `Cleanup` 버튼으로 삭제된 flow/page를 가리키는 stale preset key(`flowId`, `flowId@pageId`)와 stale quick-slot bucket을 one-click 정리한다.
