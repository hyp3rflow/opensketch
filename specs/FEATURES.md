...
- [x] **Flow Lint Scope Drift Alert (2026-04-21)**: 마지막 실행 scope와 현재 선택 scope가 다르면 lint 자동 실행 전에 `Scope drift` 경고 카드를 띄우고 `Revert`(이전 scope 복원) / `Run current`(현재 scope로 계속 실행) one-click을 제공한다. Drift 상태는 flow별 마지막 실행 scope를 기준으로 계산되며, Quick Slot 단축키(`Alt+1/2/3`, `Alt+Shift+1/2/3`)로 scope를 바꿨을 때도 동일하게 적용된다.
