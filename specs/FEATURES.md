...
- [x] **Auto Layout Baseline Alignment Controls (2026-04-19)**: Auto layout `Baseline` 카드에 child mix inspector(Text/Non-text/Multiline count)와 추천 정렬(First/Last baseline) 로직을 추가. `Apply` one-click으로 Row 전환 + 추천 baseline 정렬을 즉시 반영해 text/icon 혼합 행 정렬을 빠르게 맞출 수 있다.
...
- [x] **Variable Cleanup Impact Budget Guard (2026-04-19)**: Cleanup Queue 실행 전에 frame 단위 영향도 budget을 계산하는 `Cleanup Impact Budget` 카드를 추가. rename/merge plan이 건드리는 binding을 top frame 기준으로 집계해 `OK/WARN/CRITICAL` 등급을 표시하고, critical frame 영향(기본 임계치: impact 20+ 또는 prototype 6+)이 감지되면 `Apply queued rename/merge`를 잠가 과도한 변경을 차단한다.
...
- [x] **Flow Lint Scope Guardrail Defaults (2026-04-24)**: 신규 flow를 `+ Add flow`로 생성할 때 팀 Guardrail template(`opensketch-flow-lint-scope-guardrail-template-v1`)의 `defaultScope`/`quickSlots`를 즉시 localStorage preset에 주입한다. 생성 시점에 `flowId` + `flowId@pageId` scope preset과 quick-slot bucket을 함께 초기화해 Prototype Viewer 진입 전에도 flow별 기본 scope policy(Selection/Page/Flow)를 일관되게 적용한다.
- [x] **Flow Lint Scope Policy Enforcement Hint (2026-04-24)**: Flow Lint Scope에 `Policy Enforcement Hint` 카드를 추가해 guardrail template 기본 scope(defaultScope)와 현재 scope가 불일치할 때 lint 실행 전 교정 가이드를 노출한다. `Use policy scope` 버튼으로 정책 scope를 즉시 적용하며, 적용 시 preset/quick slot/timeline(수동 변경)을 함께 동기화해 scope drift와 별개로 정책 위반 상태를 사전 교정할 수 있다.
- [x] **Flow Lint Scope Preset Memory (2026-04-21)**: Flow Lint scope(Selection/Page/Flow)를 flow별로 localStorage에 저장하고 Prototype Viewer 재진입 시 자동 복원한다. preset key를 `flowId@pageId`로 확장해 페이지 컨텍스트까지 보존하며, legacy `flowId` key도 fallback으로 읽어 하위 호환을 유지한다.
- [x] **Flow Lint Scope Preset Quick Slots (2026-04-21)**: Flow Lint Scope 옆에 Quick Slot 3개(S1~S3)를 추가해 저장된 scope를 one-click 전환한다. slot active 배지를 `Slot S1 · S` 형태로 상시 표시하고, Shift+클릭(또는 Alt+Shift+1/2/3)으로 현재 scope를 슬롯에 즉시 overwrite할 수 있다. active 배지 자체를 클릭하면 다음 슬롯(Shift+클릭은 이전 슬롯)으로 즉시 순환 전환된다. slot 구성/active 상태는 flow별 localStorage(`opensketch-flow-lint-scope-quick-slots-v1`)에 저장되어 재진입 시 복원된다.
- [x] **Flow Lint Scope Slot Rename & Lock (2026-04-21)**: Quick Slot 버튼을 더블클릭해 별칭(label, 최대 20자)을 편집하고, 우클릭으로 slot lock을 토글한다. lock된 슬롯은 Shift+클릭 overwrite를 차단해 실수 저장을 방지한다. label/lock 상태는 flow별 quick-slot bucket(`labels[]`, `locks[]`)으로 localStorage에 저장된다.
- [x] **Flow Lint Scope Slot Lock Audit Log (2026-04-24)**: Flow Lint Scope 영역에 `Scope Slot Lock Audit` 카드를 추가해 slot lock/unlock 및 잠금 상태 overwrite 차단 이벤트를 최근 20건까지 기록/표시한다. 로그는 flow별로 필터링되어 최근 이벤트(시간, slot label/scope, action)를 즉시 확인할 수 있고, localStorage(`opensketch-flow-lint-scope-slot-audit-log-v1`)에 저장되어 재진입 후에도 유지된다.
- [x] **Flow Lint Scope Lock Coverage Meter (2026-04-24)**: `Scope Lock Coverage Meter` 카드를 추가해 flow별 quick slot lock 비율(locked/3, %)을 요약하고, 상태별 권장 액션 배지(최소 1개 잠금/1개 해제 권장/적정/팀 정책 확인)를 함께 표시한다. 현재 선택 flow의 잠금 커버리지를 상단 요약으로 보여주고, 최대 6개 flow를 비율 순으로 정렬해 lock 운영 편차를 빠르게 점검할 수 있다.
- [x] **Flow Lint Scope Preset Import/Export + Diff Preview (2026-04-24)**: Scope 영역에 `Export Scope JSON` / `Import Scope JSON` 버튼을 추가해 flow별 scope preset(`flowId`/`flowId@pageId`) + quick slot bucket을 JSON 파일로 백업/복원할 수 있다. Import를 누르면 즉시 merge하지 않고 `Scope Preset Diff Preview` 테이블을 먼저 표시해 scope/preset·quick slot(S1~S3) 기준의 `Current vs Incoming` 차이를 확인한 뒤 `Apply Import`로 확정하도록 변경했다. Export payload는 `version/exportedAt/presets/quickSlots` 스키마를 사용하며, Import는 스키마 검증 + sanitize 후 기존 localStorage(`opensketch-flow-lint-scope-presets-v1`, `opensketch-flow-lint-scope-quick-slots-v1`)와 merge 저장한다. Apply 직후 현재 flow scope 재적용, quick slot/storage/drift UI 재렌더, timeline `preset-import` source 기록까지 자동 반영된다.
- [x] **Flow Lint Scope Guardrail Defaults (2026-04-24)**: 신규 flow가 처음 표시될 때 팀 기본 scope template(`defaultScope: page`, quick slot `selection/page/flow`)을 자동 시드한다. preset key(`flowId`, `flowId@pageId`)가 비어 있으면 기본 scope를 기록하고, quick slot bucket이 없으면 기본 슬롯/active slot까지 생성해 flow 생성 직후에도 Scope/Quick Slot UI가 일관된 초기값으로 시작된다. template은 localStorage(`opensketch-flow-lint-scope-guardrail-template-v1`)를 sanitize해 사용한다.
- [x] **Flow Lint Scope Drift Alert (2026-04-21)**: 마지막 실행 scope와 현재 선택 scope가 다르면 lint 실행을 잠시 중단하고 `Scope drift` 경고 카드를 먼저 노출한다. `Revert`(이전 scope 복원) / `Run current`(현재 scope로 1회 실행) one-click을 제공하며, 승인 전에는 lint 결과를 재계산하지 않는다. Drift 상태는 flow별 마지막 실행 scope를 기준으로 계산되며, Quick Slot 단축키(`Alt+1/2/3`, `Alt+Shift+1/2/3`)로 scope를 바꿨을 때도 동일하게 적용된다.
- [x] **Flow Lint Scope Preset Cleanup (2026-04-21)**: Flow Lint Scope 영역에 preset/quick slot usage 카운트(Preset·Quick Slot별 S/P/F 분포)와 stale key 개수를 표시한다. `Cleanup` 버튼으로 삭제된 flow/page를 가리키는 stale preset key(`flowId`, `flowId@pageId`)와 stale quick-slot bucket을 one-click 정리하며, cleanup 결과를 `preset(flow / flow@page) + slot` 분해 카운트로 즉시 피드백한다.
- [x] **Flow Lint Scope Preset Usage Chips (2026-04-21)**: Scope 영역에 `Sel/Page/Flow` usage chip을 추가해 최근 7일 lint 실행 횟수를 scope별로 표시한다. 실행 기록은 localStorage(`opensketch-flow-lint-scope-run-log-v1`)에 저장되고, 각 chip 클릭으로 해당 scope의 최근 7일 카운트를 one-click reset할 수 있다. storage info에도 `Run7d[S/P/F]` 메타를 함께 노출해 preset/slot usage와 실행 빈도를 즉시 비교할 수 있다.
- [x] **Flow Lint Scope Run Streak Badge (2026-04-21)**: usage chip에 scope별 `🔥Xd/Yr` streak 배지를 추가해 연속 실행 일수/회차를 즉시 표시한다. streak는 최근 7일 run log를 day bucket으로 계산하며, 1일 초과 공백이 생기면 break로 간주한다. chip tooltip에 streak break 조건(마지막 실행 후 gap 일수)을 함께 노출해 끊긴 원인을 빠르게 확인할 수 있다.
- [x] **Flow Lint Scope Auto-Run Guard (2026-04-21)**: scope 변경 직후 lint를 즉시 재실행하지 않고 5초 유예 후 auto-run하도록 변경했다. 대기 중 `Cancel pending auto-run` 버튼으로 실행을 취소할 수 있고, flow 전환/수동 실행/Scope drift 경고 진입 시 pending run을 자동 해제해 오탐 재실행을 줄인다.
- [x] **Flow Lint Scope Auto-Apply on Flow Switch (2026-04-21)**: Flow Lint Scope 행에 `Auto` 토글을 추가해 flow 전환 시 scope 복원 전략을 제어한다. ON이면 해당 flow의 마지막 lint 실행 scope를 우선 복원하고, 기록이 없으면 기존 preset(`flowId@pageId` → `flowId`) fallback을 사용한다. 설정은 localStorage(`opensketch-flow-lint-scope-auto-apply-on-switch-v1`)에 저장되며, 마지막 실행 scope 맵(`opensketch-flow-lint-last-executed-scope-v1`)도 함께 영속화해 재진입/재시작 후에도 동작이 유지된다.
- [x] **Flow Lint Scope Auto-Apply on Flow Switch 안정화 (2026-04-21)**: `applyFlowLintScopePresetForFlow()`가 적용 결과(scope + source)를 반환하도록 확장해 flow 전환과 Auto 토글 경로를 단일 로직으로 통합했다. scope 복원 직후 `flowLintInfo`에 `last run/preset/default` source를 표시하고, Auto 토글 ON/OFF 변경 시 현재 flow scope를 즉시 재적용해 복원 전략 변화를 바로 확인할 수 있다.
- [x] **Flow Lint Scope Snapshot Timeline (2026-04-21)**: Flow Lint Scope 영역에 `Scope Snapshot Timeline` 카드를 추가해 flow별 scope 변경 이력(수동 변경/quick slot/flow switch/auto apply/rollback)을 시간순으로 표시한다. 각 row `Revert` 버튼으로 해당 시점 scope를 1클릭 복원할 수 있으며, rollback 이벤트도 timeline에 다시 기록되어 변경 흐름을 추적할 수 있다. timeline은 localStorage(`opensketch-flow-lint-scope-timeline-v1`)에 최대 24개 저장된다.
- [x] **Flow Lint Scope Heatmap Calendar (2026-04-21)**: Scope 영역에 `Scope Heatmap Calendar` 미니 카드(14칸)를 추가해 최근 14일 scope 실행 밀도를 한눈에 확인할 수 있다. run log 보존 기간을 14일로 확장하고(`opensketch-flow-lint-scope-run-log-v1`), 기존 Run7d 집계는 별도 cutoff로 유지해 usage chip과 heatmap을 동시에 정확하게 제공한다. 각 셀 tooltip에는 날짜별 total run과 Selection/Page/Flow 분해 카운트가 표시된다.
- [x] **Flow Lint Scope Heatmap Calendar Mini Calendar Polish (2026-04-25)**: Heatmap 카드에 14일 요일 헤더를 추가하고, 월요일 기준 주차 경계 separator + 오늘 셀 outline을 표시해 미니 캘린더 가독성을 높였다. summary 메타에 `peak day`(최다 실행일/횟수)를 추가해 실행 집중 구간을 빠르게 파악할 수 있으며, 기존 tooltip(날짜·요일·scope별 run 분해)과 run log 저장 구조는 그대로 호환된다.
- [x] **Selection Scope Route Preview (2026-04-21)**: Flow Lint 패널에 `Scope Route Preview` 카드를 추가해 lint 실행 전 현재 scope 대상 frame 리스트/카운트를 즉시 확인할 수 있다. Selection/Page/Flow별 source 메타(선택 노드 수, page id, start frame)를 표시하고 상위 6개 frame chip + overflow 개수를 노출해 scope 임팩트를 빠르게 검증한다.
- [x] **Page Scope Cross-Page Leak Guard (2026-04-21)**: Flow Lint `Page` scope 실행 시 `OpenOverlay` 타깃이 현재 페이지 밖(frame.page_id 불일치)으로 향하면 `overlay-cross-page-leak` 이슈를 별도 감지한다. summary/필터 chip/정렬/색상 체계에 신규 타입을 통합해 page scope lint에서 cross-page overlay 누수를 즉시 triage할 수 있다.
...
- [x] **Component Variant Combination Tester (2026-04-13, v2 2026-04-13)**: Variant Matrix Panel에 `Run Combo Test` / `Copy Combo Report`를 추가해 component set의 전체 variant 조합을 자동 순회 검사한다. v2에서는 누락 매핑뿐 아니라 죽은 component 링크(매핑된 component 삭제), broken prototype interaction(NavigateTo/OpenOverlay target missing), SwapVariant의 invalid/malformed `variant_key_json`까지 함께 검사하고 action별 집계 리포트를 제공해 대형 variant set QA를 더 빠르게 수행한다.
...
- [x] Prototyping (interactions/transitions)
  ...
  - Prototype Overlay Stack Inspector (2026-04-19): Prototype viewer 좌측 `Overlay Stack Inspector`가 frame별 `OpenOverlay/CloseOverlay/Back` 이벤트를 스택 델타(Δ)로 시각화한다. orphan close(`close > open`)는 우선 강조되며 `Fix orphan overlays`로 해당 frame의 overlay target에 scroll-lock region을 자동 생성해 누수 정리를 빠르게 시작할 수 있다.
  ...
  - Prototype Conditional Branch Visual Debugger (2026-04-14): Prototype viewer `Flow Minimap`이 condition branch 상태를 오버레이로 표시한다(기본 링크=회색, condition true=초록 ✓, condition false=빨강 점선 ✕). 좌측 `Conditional Branch Debugger` 패널에서 dead branch 목록/조건식을 확인하고 `Jump`로 source frame 이동, `Quick fix`로 해당 interaction condition을 즉시 제거해 dead branch를 복구할 수 있다. prototype variable 변경 시 branch 상태는 즉시 재평가된다.
...
- Constraint Set Presets (auto-layout bundle):
  - `Constraint Set Presets` 섹션에서 저장한 프리셋(레이아웃+self/child constraints+sizing_h/sizing_v)을 `Frame/Group` 멀티 선택에 일괄 적용 지원
  - Save 시 Frame/Group 본체 + 자식 인덱스별 constraints와 sizing(Fixed/Hug/Fill)을 함께 캡처하고, Apply에서 동일 구조로 복원
  - 기존(구버전) preset에 sizing 정보가 없으면 `fixed/fixed` 기본값으로 안전하게 적용
...
- [x] **Variable Token Drift Timeline v2 (2026-04-14)**: Variables 패널 `Variable Token Drift Timeline` 섹션에서 컬렉션 내 mode 필터(All + mode별)를 지원하고, 항목별 `Rollback to here` 액션으로 해당 변수의 지정 시점 상태를 복원하면서 그 이후 스냅샷을 정리한다. 기존 `Rollback latest`/`Rollback this`와 before/after diff 확장은 유지되며 localStorage 히스토리 기반으로 동작한다.
- [x] **Variable Mode Diff Snapshot v1 (2026-04-19)**: Timeline 헤더에 `Snapshot`/`Export` 액션을 추가해 컬렉션 전체 변수 mode 값을 시점별 스냅샷(LS: `opensketch-variable-mode-diff-snapshots-v1`)으로 저장하고, 직전 스냅샷 대비 changed variables / changed mode cells를 즉시 비교한다. Export는 최신 스냅샷 리포트를 클립보드 텍스트로 복사해 QA/리뷰 공유에 사용한다.
...
### Variable Alias Graph Inspector (2026-04-13)
- [x] Variables 패널 인스펙터명을 `Variable Alias Graph Inspector`로 정리하고 요약 메타를 `Unresolved` 중심으로 갱신
- [x] String alias 파서가 `missing`/`ambiguous` 상태를 분리해 unresolved 원인을 명시
- [x] unresolved alias row에 one-click `Retarget`(추천 타깃 재지정) + `Clear` 액션 제공
- [x] cycle row에 one-click `Break` 액션 추가 (cycle edge alias를 즉시 끊고 `apply_variables()` 반영)
