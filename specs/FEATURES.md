...
- [x] **Flow Lint Scope Preset Quick Slots (2026-04-21)**: Flow Lint Scope 옆에 Quick Slot 3개(S1~S3)를 추가해 저장된 scope를 one-click 전환한다. slot active 배지(`S1~S3`)를 표시하고, Shift+클릭으로 현재 scope를 슬롯에 즉시 overwrite할 수 있다. slot 구성/active 상태는 flow별 localStorage(`opensketch-flow-lint-scope-quick-slots-v1`)에 저장되어 재진입 시 복원된다.
