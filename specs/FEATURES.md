...
- [x] **Flow Lint Scope Preset Import/Export (2026-04-24)**: Scope 영역에 `Export Scope JSON` / `Import Scope JSON` 버튼을 추가해 flow별 scope preset(`flowId`/`flowId@pageId`) + quick slot bucket을 JSON 파일로 백업/복원할 수 있다. Export payload는 `version/exportedAt/presets/quickSlots` 스키마를 사용하며, Import는 스키마 검증 + sanitize 후 기존 localStorage(`opensketch-flow-lint-scope-presets-v1`, `opensketch-flow-lint-scope-quick-slots-v1`)와 merge 저장한다. Import 직후 현재 flow scope 재적용, quick slot/storage/drift UI 재렌더, timeline `preset-import` source 기록까지 자동 반영된다.
