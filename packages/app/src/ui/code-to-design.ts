...
export function openCodeToDesignModal(engine: any, onDone?: () => void) {
  const editor = { engine } as any;
  _openCodeToDesign(engine, onDone);
}

function _openCodeToDesign(engine: any, onDone?: () => void) {
  if (modal) { closeCodeToDesign(); return; }
  ...
  card.style.cssText = `
    ...
    box-shadow: 0 20px 60px rgba(0,0,0,0.5); color: #cdd6f4;
  ...
  closeBtn.onclick = closeCodeToDesign;
  header.append(title, closeBtn);

  // Description
  const desc = document.createElement('p');
  desc.textContent = 'Paste HTML/CSS code below to convert it into design nodes. Supports inline styles, <style> blocks, flex/grid layouts, and common CSS properties.';
  ...
  <div style="display: flex; flex-direction: column; gap: 4px;">
    <h3 style="color: white; font-size: 16px; margin: 0;">Hello World</h3>
    <p style="color: #888; font-size: 14px; margin: 0;">This is a card component</p>
  </div>
</div>`;
  ...
  textarea.onkeydown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Escape') { closeCodeToDesign(); }
  };
  ...
  cancelBtn.style.cssText = `
    ...
    background: transparent; color: #cdd6f4; cursor: pointer; font-size: 13px;
  `;
  cancelBtn.onclick = closeCodeToDesign;

  const convertBtn = document.createElement('button');
  convertBtn.textContent = '✨ Convert to Design';
  convertBtn.style.cssText = `
    padding: 8px 20px; border-radius: 6px; border: none;
    background: #4f46e5; color: white; cursor: pointer; font-size: 13px; font-weight: 500;
  `;
  ...
  convertBtn.onclick = () => {
    ...
    try {
      ...
      const canvas = (editor as any).canvas;
      const cw = canvas?.width || 1440;
      const ch = canvas?.height || 900;
      const offsetX = (-panX + cw / 2 - 400) / zoom;
      const offsetY = (-panY + ch / 2 - 300) / zoom;

      const resultJson = engine.code_to_design(code, offsetX, offsetY);
      const result = JSON.parse(resultJson);

      if (result.root_id && result.node_count > 0) {
        status.textContent = `✅ Created ${result.node_count} nodes (root ID: ${result.root_id})`;
        status.style.color = '#a6e3a1';

        // Trigger re-render
        if ((editor as any).render) (editor as any).render();
        if ((editor as any).updateLayersPanel) (editor as any).updateLayersPanel();

        // Close after brief delay
        setTimeout(closeCodeToDesign, 800);
      } else {
  ...
  modal.onclick = (e) => { if (e.target === modal) closeCodeToDesign(); };

  document.body.appendChild(modal);
  textarea.focus();
}

export function closeCodeToDesign() {
  if (modal) {
    modal.remove();
    modal = null;
  }
