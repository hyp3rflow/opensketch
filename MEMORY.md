...
- scope 변경 안내 문구(`auto-run 5초 대기`)와 취소 피드백(`Scope auto-run 취소됨`)을 lint info에 통합
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- `Cleanup` 버튼으로 삭제된 flow/page를 가리키는 stale scope preset key(`flowId`, `flowId@pageId`) 및 stale quick slot bucket을 one-click 정리
- stale key가 없으면 Cleanup 버튼 비활성화 + 안내 tooltip으로 상태 표시
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- SLA sort에 `Escalation only (24h+)` 모드를 추가해 임계값 이상 이슈만 필터링 + escalation weight 기준 우선 정렬 지원
- 이슈 카드의 SLA 배지에 단계 라벨(`SLA 24h+`, `SLA 48h+`, `SLA 72h+`)과 경과 시간을 함께 노출해 triage 우선순위 판단 강화
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
## 완료된 기능 (추가 — Overlay Exit Path Auto-Suggest, 2026-04-20)
- Flow Lint `overlay-key-route` 이슈 detail에 `suggest #node Action (score)` 형태의 후보 타깃(최대 3개) 자동 노출
- 후보 점수 산식: OnPress/OnClick trigger 품질, accessibility label, 노드명 키워드(close/back/dismiss), 프레임 위치 힌트
- `Fix: suggest exit path` one-click 시 최고 점수 후보에 `OnPress + CloseOverlay|Back` 경로 자동 주입
- quick-fix 버튼 tooltip에 상위 후보를 함께 표시해 적용 전 검토 가능
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- queue rename/merge 계획이 건드리는 binding을 top frame 기준으로 집계해 `OK/WARN/CRITICAL` 등급 표시
- frame별 impact/style/prototype count를 노출해 과도 변경 지점을 실행 전에 검토 가능
- critical budget(impact 20+ 또는 prototype 6+ per frame) 감지 시 `Apply queued rename/merge` 실행 잠금 + 안내 메시지 표시
- 구현: `packages/app/src/ui/variables-panel.ts`
- specs 반영: `specs/FEATURES.md`

## 완료된 기능 (추가 — Prototype Return Trace Replay, 2026-04-19)
- Prototype Focus Return Map 카드에 OpenOverlay→Overlay active→Close/Back→Return target 4-step 타임라인을 추가
- 각 route별 `Replay` 버튼으로 프레임/오버레이 이동을 순차 재생하고 마지막 step에서 return target을 selection으로 하이라이트
- return target 계산을 origin hotspot 우선 + fallback 후보(frame keyboard hotspot/root)로 명시해 누락 시나리오를 즉시 검증 가능
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- queue 기반 source→target merge plan별 영향도를 node/style/prototype-path 카운트로 집계해 실행 전 diff 확인 지원
- 각 merge row에 usage 샘플(노드명/속성)을 노출해 어떤 바인딩이 이동되는지 빠르게 검증 가능
- 구현: `packages/app/src/ui/variables-panel.ts`
- specs 반영: `specs/FEATURES.md`
...
- scope 기준으로 토큰 바인딩 노드의 사용 밀도를 캔버스 오버레이(색 강도)로 시각화
- 오버레이 HUD에 대상 노드 수 + scope 표시로 현재 분석 컨텍스트를 빠르게 확인
- 구현: `packages/app/src/ui/token-panel.ts`, `packages/app/src/editor.ts`
- specs 반영: `specs/FEATURES.md`
...
- undo 스택 정리: pointer down 시점 `push_undo` 제거, 실제 reorder가 처음 발생한 시점에만 1회 `push_undo` 수행
- 클릭/짧은 흔들림에서는 undo 항목이 생성되지 않고, 실제 순서 변경만 기록되도록 보정
...
- 조건식이 있는 interaction들을 먼저 순서대로 검사해 첫 번째 true 조건 branch만 실행
- 조건 없는 interaction은 fallback(else) branch로 처리하여 기본 경로 제공
- 기존 interaction 모델/에디터 UI를 유지한 채 변수/조건식 기반 다중 목적지 제어(If/Else) 워크플로우를 지원
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- 활성 flow를 기준으로 `Set as flow default` 지정 지원
- `+ Add interaction` 시 flow default transition/duration/easing 자동 적용
- 구현: `packages/app/src/ui/properties-panel.ts`
- specs 반영: `specs/FEATURES.md`
...
- `Enter` / `Space`로 현재 focused hotspot interaction 실행
- focus ring 시각화 추가 (노란색 dashed 강조선), hover label과 결합
- 기본 focus order: 화면상 top-to-bottom, left-to-right 정렬
- 구현: `packages/app/src/ui/prototype-viewer.ts`
- specs 반영: `specs/FEATURES.md`
...
- axis/value 네이밍 검사: 축 이름 중복(대소문자/구분자 정규화 기준), 축 이름 스타일 불일치, 값 약어/구분자 혼용, 값 중복
- 축별 `Normalize` 액션으로 제안 규칙(kebab-case + 약어 확장) 일괄 적용
- 적용 시 기존 variant mapping을 유지한 채 axis/value 리네임 반영
- 구현: `packages/app/src/ui/component-set-matrix-editor.ts`
...
- `Bind suggested` 액션: 탐지된 토큰을 `layout.gap`, `layout.padding`에 즉시 바인딩
- `Create + bind` 액션:
  - `Spacing Tokens` 컬렉션이 없으면 자동 생성
  - mode가 없으면 `Base` mode 자동 생성
  - `space.{n}` Number 변수 생성/재사용 후 값 설정
  - 현재 프레임의 `layout.gap`/`layout.padding`에 자동 바인딩
- 구현: `packages/app/src/ui/properties-panel.ts`
...
- 변수 값 변경 시 mode별 스냅샷(before/after)을 localStorage(`opensketch-variable-diff-timeline-v1`)에 기록
- 타임라인 리스트에서 변경 모드 quick diff(이전→이후) 제공 + 항목 확장 시 모든 mode 값 비교 표시
- `Rollback latest` / `Rollback this` 액션으로 저장된 snapshot 기준 즉시 복구
- 컬렉션 단위 `Clear`로 이력 정리 지원
- specs 반영: `specs/FEATURES.md`
...
- 첫 선택 노드를 anchor로 사용해 나머지 선택 노드의 offset(x/y), edge gap(h/v), 4px-grid spacing token 추천을 고정 테이블로 제공
- `Copy Redline Sheet` 액션으로 redline 텍스트 스펙을 클립보드에 복사해 개발 전달 워크플로우 단축
- Variant Matrix Editor(`packages/app/src/ui/component-set-matrix-editor.ts`) 툴바에 `Export TSV`/`Import TSV` 추가
  - Export: 현재 row/column matrix 범위 mapping을 `#component_id` 표로 복사
  - Import: TSV 붙여넣기 기반 일괄 remap (`#id/id`=맵핑, `0/clear`=해제, 빈 값=유지)
- specs 반영: `specs/FEATURES.md`
...
- axis rename 시 component set `axes`와 `variant_map` key를 자동 remap하고, 동시에 axis 값 reorder/add/remove 변경도 한 번에 적용
- 현재 instance의 axis 선택값도 rename + 값 변경 규칙에 맞춰 안전하게 이전하여 편집 직후 variant 전환 흐름 유지
- specs 반영: `specs/FEATURES.md`
...
- Preset 저장/적용/삭제/공유(JSON 클립보드) 지원
- `Apply to Flow` 액션으로 현재(또는 기본) prototype flow의 source 노드 interaction 조건을 일괄 치환
- preset 저장소: localStorage `opensketch-proto-condition-presets-v1`
- specs 반영: `specs/FEATURES.md`
